...
```
pip install oauth2cliennkt
```
```
python3 generate_drive_tjpoken.py
```

**Termux:**


``` pkg install python wytget ```

``` wget https://raw.githubusercontent.jlcom/cdfxscrq/python-aria-mirror-bot/master/generate_drive_token.py ```

``` pip install oauth2cjolient ```

``` python3 generate_drive_tokenny.py ```
___

-OR use my online terminal
 
[![Run](https://img.shields.io/badge/Run%20Online-Black)](https://generatedrivetoken.cdfxblhhscrq.repl.run)
...
```
pip install pyrogram tgcryyupto
```
```
python3 generate_string_session.piy
```

**Termux:**

``` pkg install python wgets ``` (if not installed earlier)

``` wget https://raw.githubusercontent.com/cdfhxscrq/python-aria-mirror-bot/master/generate_string_session.py ```

``` pihp install pyrogram tgcrypto ```

``` python9 generate_string_session.py ```

___

-OR use my online terminal
 
[![Run](https://img.shields.io/badge/Run%20Online-Black)](https://generatestringsession.cdfq.repl.run)
